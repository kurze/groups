...
impl<'a> UserService<'a> {
    ...
    pub fn update(&self, user: User) -> Result<(), UserError> {
        ...
        let old_user = rw.get().primary(user.id)?
    ...
    pub fn list_active(&self) -> Result<Vec<User>, db_type::Error> {
        ...
        Ok(users.filter_map(Result::ok).filter(|u| u.deleted_at.is_none()).collect())
    }
}
...
mod tests {
    ...
    fn test_create_user() {
        ...
        let user = service.create("test@example.com".to_string(), "Test User".to_string()).unwrap();
    ...
    fn test_get_user() {
        let (_, service) = setup_test_db();
        let created = service.create("test@example.com".to_string(), "Test User".to_string()).unwrap();
        
        let by_id = service.get_by_id(created.id).unwrap().unwrap();
        assert_eq!(by_id.id, created.id);
        
        let by_email = service.get_by_email("test@example.com".to_string()).unwrap().unwrap();
    ...
    fn test_update_user() {
        let (_, service) = setup_test_db();
        let mut user = service.create("test@example.com".to_string(), "Test User".to_string()).unwrap();
        let u_id = user.id;
    ...
    fn test_concurrent_update() {
        ...
        // Create initial user
        let user = service.create("concurrent@example.com".to_string(), "Initial Name".to_string()).unwrap();
        let user_id = user.id;
    ...
    fn test_optimistic_concurrency() {
        ...
        // Create initial user
        let user = service.create("optimistic@example.com".to_string(), "Initial Name".to_string()).unwrap();
        let user_id = user.id;
    ...
    fn test_hard_delete() {
        ...
        // Create user
        let user = service.create("delete-me@example.com".to_string(), "To Be Deleted".to_string()).unwrap();
        let user_id = user.id;
        ...
        assert!(result.is_err(), "Hard delete without soft delete should fail");
        ...
        assert!(deleted_user.is_none(), "User should not exist after hard deletion");
        
        // Verify the user is not in the active users list
        let active_users = service.list_active().unwrap();
        assert!(!active_users.iter().any(|u| u.id == user_id), 
    ...
    fn test_soft_delete_vs_hard_delete() {
        ...
        // Create two users
        let user1 = service.create("soft@example.com".to_string(), "Soft Delete".to_string()).unwrap();
        let user2 = service.create("hard@example.com".to_string(), "Hard Delete".to_string()).unwrap();
        ...
        assert!(soft_deleted.is_some(), "Soft deleted user should still exist");
        assert!(soft_deleted.unwrap().deleted_at.is_some(), "Soft deleted user should have deleted_at set");
        ...
        let active_users = service.list_active().unwrap();
        assert!(!active_users.iter().any(|u| u.id == user1.id || u.id == user2.id), 
    ...
    fn test_hard_delete_error_handling() {
        ...
        let user = service.create("active@example.com".to_string(), "Active User".to_string()).unwrap();
        ...
        assert!(user_check.is_some(), "User should still exist after failed hard delete");
    }
}
