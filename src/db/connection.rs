        ...
        for statement in statements {
            if !statement.trim().is_empty() {
                sqlx::query(statement)
                    .execute(pool)
                    .await
                    .map_err(|e| DatabaseError::Migration(format!("Migration {} statement '{}' failed: {}", i + 1, statement, e)))?;
            }
        }
    }
...
pub async fn create_test_pool() -> DbResult<DbPool> {
    let database_url = std::env::var("DATABASE_TEST_URL")
        .unwrap_or_else(|_| "postgresql://groups_user:groups_password@localhost:5433/groups_test".to_string());
